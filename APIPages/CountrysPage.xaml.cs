...
namespace EnergyScanVerwaltung
{
    ...
    /// </summary>
    public sealed partial class CountrysPage : Page
    {
        public ObservableCollection<Country> Countrys { get; } = new ObservableCollection<Country>();
        public CountrysPage()
        {
            InitializeComponent();
        ...
        {
            Countrys.CollectionChanged += new NotifyCollectionChangedEventHandler(RefreshListview);
            Task<bool> task = GetCountrys();
        }
        private void CountryEditButton_Click(object sender, RoutedEventArgs e)
        {
            Button b = sender.GetType().Equals(typeof(Button)) ? sender as Button : null;
            if (b != null)
            {
                foreach (Country obj in Countrys)
                {
        ...
        private void CountryDeleteButton_Click(object sender, RoutedEventArgs e)
        {

        }

        private void CountryAddButton_Click(object sender, RoutedEventArgs e)
        {
        ...
        private async Task<bool> GetCountrys()
        {
            ...
            if (response.StatusCode.Equals(HttpStatusCode.OK))
            {
                try
                {
                    ...
                    for (int i = 0; i < b.Count; i++)
                    {
                        ...
                        Countrys.Add(new Country(c.GetValueOrDefault("id"), c.GetValueOrDefault("name")));
        ...
        private async void AddButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            Country s = new Country() { name = addName.Text, id = "" };
            ...
            var response = await client.PostAsync(new Uri(Root.Config.Route["API:BaseUrl"] + "/country"), data);
